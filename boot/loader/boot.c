...
void boot_main() {
    ...
    if (cpu.sse)  ok("SSE supported");
    if (cpu.sse2) ok("SSE2 supported");
    if (cpu.nx)   ok("NX bit supported");
    ...
    ElfInfo elf;
    if (!elf_load(KERNEL_LOAD_ADDR, &elf)) {
    ...
    ok("Jumping to kernel...");
    KernelEntry entry = (KernelEntry)(uint32_t)elf.entry;
    entry(&boot_info);
